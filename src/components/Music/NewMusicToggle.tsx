  ...
  return (
    <button className="cursor-pointer absolute top-1/12 md:top-1/8 right-5/7 md:right-48 z-50 h-10 w-20 md:h-12 border-2 border-purple-500 rounded-full p-1 md:p-2">
      <div
        ...
        cursor-pointer transition-all duration-500 ease-in-out z-20 flex justify-center items-center ${isPlaying ? "left-[56%]" : "left-[8%]"
          }`}
      ></div>
      ...
      {/* Unmuted Animation */}
      <div
        className={`absolute rotate-180 left-2 md:left-4 top-1/2 transform -translate-y-1/2 flex gap-0.5 md:gap-1 z-10 ${isPlaying ? "flex-row" : "hidden"
          }`}
      >
